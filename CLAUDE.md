...
- **Ops audit: every new ops action MUST write an `ops_events` entry** via `logOpsEvent()` in the same commit. Record before/after values for status and money fields. Deal-specific ops actions must also write a deal-scoped `events` entry. If the audit write fails, the action fails — never silently swallow.
