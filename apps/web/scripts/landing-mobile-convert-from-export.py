#!/usr/bin/env python3
"""
Convert the mobile Landing export into the JSX body of
app/creators/LandingMobileClient.tsx.

Run from apps/web:  python3 scripts/landing-mobile-convert-from-export.py
...
html = open(f"{S}/landing-mobile-body.html").read()
ASSETS = json.load(open(f"{S}/landing-mobile-assets.json"))
...
# The same four the desktop landing page hides: recent deals and featured
# campaigns have no real data behind them (and the export's are invented), and
# the creator roster and brand logos are still behind their feature flags. Index-based and
# back to front, because every offset after a removal shifts.
HIDE = ['Recent deals on', 'Featured', 'Find vetted', 'Trusted by']
spans = []
for needle in HIDE:
    at = html.find(needle)
    if at == -1:
...
before = html.count('id="landProgress"')
html = ('<div id="landProgress" style="position:fixed;top:0;left:0;height:2px;width:0%;'
        'background:var(--neon-deep);z-index:200;transition:width .1s linear;"></div>\n' + html)
step("scroll progress added", before, html.count('id="landProgress"'))
...
# ── 11d. The FAQ loop. ────────────────────────────────────────────────────
# The export binds it with <sc-for list={faqs} as="f">, which React would render
# as an unknown element. It becomes a real map, and the toggle becomes real
# state. The desktop landing page uses the same FAQ constant, so both layouts of
# this page answer identically.
def convert_sc_for(src):
    count = 0
    while True:
        m = re.search(r'<sc-for\s+list=\{(\w+)\}\s+as="(\w+)"[^>]*>', src)
        if not m:
            break
        lst, alias = m.group(1), m.group(2)
        depth, k = 1, m.end()
        while depth:
            nxt = re.search(r'<sc-for\b|</sc-for>', src[k:])
            k += nxt.end()
            depth += 1 if nxt.group(0).startswith('<sc-for') else -1
        inner = src[m.end():k - len('</sc-for>')]
        src = src[:m.start()] + '{' + lst + '.map((' + alias + ', i) => (' + inner + '))}' + src[k:]
        count += 1
    return src, count
before = html.count('<sc-for')
html, _n = convert_sc_for(html)
html = re.sub(r'onClick=\{\w+\.toggle\}', 'onClick={() => setOpenFaq(openFaq === i ? -1 : i)}', html)
step("FAQ loop → map", before, html.count('<sc-for'))
...
def _img(m):
    tag = m.group(0)
    src = re.search(r'src="(/landing-mobile/[^"]+)"', tag)
...
out = f"{S}/landing-mobile-final.txt"
