#!/usr/bin/env python3
"""
Scope the mobile For Brands export's stylesheet to .brands-mobile and write
app/brands-mobile.css.

Run from apps/web:  python3 scripts/brands-mobile-scope-css.py
...
rather than a page:
  ...
  body/html      the export paints them #EDEDE8 and centres the frame with
                 flex. Scoped onto .brands-mobile that would tint our page
...
OUT = os.path.join(os.path.dirname(__file__), '..', 'app', 'brands-mobile.css')

body = open(f"{S}/brands-mobile-body.html").read()
...
HEADER = """/* ── For Brands (mobile), ported from the design export ───────────────────
 *
 * GENERATED by scripts/brands-mobile-scope-css.py — edit the script, not this
 * file.
 *
 * Shown below 820px; the desktop port in brands-page.css takes over above it.
 * Every selector is scoped to .brands-mobile so the two layouts cannot reach
...
/* The export's body rule centred the frame with flex and painted the page grey.
   Scoped onto .brands-mobile that tints our page; the layout belongs to the
...
.brands-mobile .snap-track,
.brands-mobile .creatorsTrack,
.brands-mobile [style*="overflow-x:auto"] {
  touch-action: pan-x;
...
for l in leaked[:6]:
    print("      ", l[:70])
print(f"    written: app/brands-mobile.css ({os.path.getsize(OUT):,}b)")
