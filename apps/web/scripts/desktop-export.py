#!/usr/bin/env python3
"""
Convert a DESKTOP design export into a scoped component stylesheet + JSX.

ONE script for every export. There were three near-identical copies, and the
same three bugs had to be fixed by hand in each; a fix landing in one and
silently not the others was a matter of time.

    python3 scripts/desktop-export.py \
      --src  "~/Downloads/Brand Deals - Empty Standalone.html" \
      --scope .bdeals-desk \
      --out   app/deals \
      --css   brand-deals-empty.css

Everything it knows, it learned the hard way:

  * a BRACE WALKER, not a line matcher. Exports are full of minified rules like
    `.g-card{...}` that a line-based scoper never sees. 41 leaked once.
  * comments lifted OUT of the prelude before anything reads it as a selector,
    or they get split on their own internal commas and produce
    `.scope <comment> .scope` -- a descendant selector that never matches and
    silently kills every token block.
  * entity-aware declaration splitting. The ";" ending &quot; is not a
    separator, and unescaping globally BEFORE the style pass breaks the
    style="..." attribute at the first inner quote.
  * `disabled=""` is disabled={""} in JSX, which is FALSY, and ships an
    interactive control the design drew as inert.
  * links checked AFTER the nav is stripped, so the warning reports what would
    really 404 rather than crying wolf.
  * a NOTE when content is hidden behind a class only script applies: the
    reveal never runs, and the page renders blank while every check passes.

Fatal checks abort. A converter that warns and writes the file anyway is a
converter whose warnings nobody reads.
"""
...
ap.add_argument('--src', required=True, help='the exported .html')
ap.add_argument('--scope', required=True, help='CSS scope class, e.g. .bdeals-desk')
ap.add_argument('--out', required=True, help='component dir, relative to apps/web')
ap.add_argument('--css', required=True, help='stylesheet filename')
args = ap.parse_args()
...
(OUT_DIR / args.css).write_text(css.strip() + '\n')
...
print('\n'.join(steps))
print(f'\n  css  -> {OUT_DIR / args.css}  ({len(css)} chars)')
