#!/usr/bin/env python3
"""
Convert the DESKTOP "Creator Inbox - Empty" export into a component.

Sibling of creator-deals-desktop-from-export.py and inherits every trap that one
paid for:

  * a BRACE WALKER, not a line matcher -- the exports are full of minified rules
    like `.g-card{...}` that a line-based scoper never sees;
  ...
  * `disabled=""` converted to a real JSX boolean, because disabled={""} is
    falsy and ships an interactive control the design drew as inert.

Every check here is FATAL. A converter that warns and writes the file anyway is
a converter whose warnings nobody reads.

Run:  python3 scripts/creator-inbox-desktop-from-export.py <export.html>
"""
...
SRC = Path(sys.argv[1] if len(sys.argv) > 1
           else '/Users/palakjain/Downloads/Creator Inbox - Empty Standalone.html')
OUT_DIR = Path(__file__).resolve().parent.parent / 'app' / 'creator' / 'inbox'
...
(OUT_DIR / 'inbox-empty-desktop.css').write_text(css.strip() + '\n')
...
print('\n'.join(steps))
print(f'\n  css  -> {OUT_DIR / "inbox-empty-desktop.css"}  ({len(css)} chars)')
