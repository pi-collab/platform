#!/usr/bin/env python3
"""
Convert the mobile "For Creators" export into the JSX body of
app/creators/CreatorsMobileClient.tsx.

Run from apps/web:  python3 scripts/creators-mobile-convert-from-export.py
...
html = open(f"{S}/creators-mobile-body.html").read()
ASSETS = json.load(open(f"{S}/creators-mobile-assets.json"))
...
HIDE = ['Made for creators like', 'Trusted by']
spans = []
for needle in HIDE:
    at = html.find(needle)
    if at == -1:
...
def _img(m):
    tag = m.group(0)
    src = re.search(r'src="(/creators-mobile/[^"]+)"', tag)
...
out = f"{S}/creators-mobile-final.txt"
