#!/usr/bin/env python3
"""
Convert the DESKTOP "Creator Deals - Empty" export into a component.

Sibling of creator-dashboard-desktop-from-export.py, which did the same job for
the dashboard's empty state, and it inherits that script's hard-won ordering:
sc-camel BEFORE the SVG attribute pass, explicit empty pairs collapsed BEFORE
the blanket self-closing pass. Both of those fail silently when reversed.

Like the dashboard export and unlike the mobile ones, this has no 390x844 phone
frame to strip, no sc-for loops and no {{ bindings }} -- it is static markup.

The CSS is KEPT and scoped under .cdeals-desk. It ships :root blocks, a body
reset and bare `a` rules; unscoped, those redefine the palette for the whole
site.

Run:  python3 scripts/creator-deals-desktop-from-export.py <export.html>
"""
...
SRC = Path(sys.argv[1] if len(sys.argv) > 1
           else '/Users/palakjain/Downloads/Creator Deals - Empty Standalone.html')
OUT_DIR = Path(__file__).resolve().parent.parent / 'app' / 'creator' / 'deals'
...
(OUT_DIR / 'deals-empty-desktop.css').write_text(css.strip() + '\n')
(OUT_DIR / '_desktop-body.jsx.txt').write_text(doc.strip() + '\n')

print('\n'.join(steps))
print(f'\n  css  -> {OUT_DIR / "deals-empty-desktop.css"}  ({len(css)} chars)')
