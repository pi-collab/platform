...
export default async function CreatorInboxPage() {
  ...
  const { data: messages } = await supabase
    .from('messages')
    .select('id, deal_id, sender_party, body, created_at, deals(id, title, status, brands(name))')
    .order('created_at', { ascending: false })

  const threadMap = new Map<string, { dealId: string; dealTitle: string; dealStatus: string; brandName: string; lastMessage: string; senderParty: string; createdAt: string }>()
