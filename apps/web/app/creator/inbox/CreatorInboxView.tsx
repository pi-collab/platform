...
export default function CreatorInboxView({
  ...
  const filteredThreads = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter((t) => (t.brandName + ' ' + t.dealTitle).toLowerCase().includes(q))
    }
    return list
  ...
  if (threads.length === 0) {
    return (
      <div style={{ maxWidth: 1280, margin: '0 auto', width: '100%', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 40 }}>
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', width: '100%', padding: 'clamp(14px,2vw,22px) clamp(14px,2.4vw,26px) clamp(16px,2vw,24px)', boxSizing: 'border-box', overflow: 'hidden' }}>
    <div style={{
              ...
              <>
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-hairline, #EAEAE3)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
                    <a
                      href={`/creator/deals/${selectedThread.dealId}`}
                      style={{
                        ...
                        return (
                          <div key={msg.id} style={{ display: 'flex', justifyContent: isCreator ? 'flex-end' : 'flex-start' }}>
                            <div style={{ maxWidth: '74%' }}>
                              <div style={{
                                padding: '11px 15px',
                                borderRadius: isCreator ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                                background: isCreator ? '#16170F' : 'var(--sec-2, #f5f5f0)',
                                color: isCreator ? 'var(--card)' : 'var(--ink)',
                                fontFamily: 'var(--font-ui)', fontSize: 14, lineHeight: 1.5,
                                whiteSpace: 'pre-wrap', wordBreak: 'break-word',
                                ...(isCreator ? {} : { boxShadow: '0 8px 20px -16px rgba(40,45,25,.4)' }),
                              }}>
                                {msg.body}
                              </div>
                              <div style={{
                                fontFamily: 'var(--font-ui)', fontSize: 11, color: 'var(--ink-faint)',
                                marginTop: 5, textAlign: isCreator ? 'right' : 'left',
                              }}>
                                {isCreator ? 'You' : 'Brand'} · {new Date(msg.created_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                              </div>
                            </div>
                          </div>
                        )
