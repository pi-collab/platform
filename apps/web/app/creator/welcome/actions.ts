...
export async function saveOnboardingAnswers(input: OnboardingAnswers): Promise<SaveResult> {
  ...
  for (const key of ['biggest_pain', 'deal_handling', 'monthly_deals'] as const) {
  ...
  const { error } = await admin.from('creator_onboarding_responses').insert({
    ...
    biggest_pain: input.biggest_pain,
    ...
    pain_other: input.biggest_pain === 'other' && painOther ? painOther : null,
