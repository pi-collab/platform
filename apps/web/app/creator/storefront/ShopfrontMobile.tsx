...
export default function ShopfrontMobile({
  ...
  const contentItems = data.contentItems.slice(0, 5).map((c: ContentItem) => ({
    ...
    slot: c.thumbnailUrl ?? '',
    // The card renders this through the same control the desktop card uses, so
    // a clip gets a play button rather than being fed to <img>, which is what
    // produced a broken-image icon on video items.
    raw: c,
              ...
              <div className="sr">
                ...
                <div className="mcard" style={{marginTop: '16px', padding: '24px 22px'}}>
                  ...
                  <div style={{display: 'flex', alignItems: 'flex-end', gap: '16px', marginTop: '22px'}}>
                    <div style={{flex: '1', textAlign: 'center'}}><div style={{fontWeight: '700', fontSize: '13px', color: 'var(--ink)', marginBottom: '6px'}}>{age[0] ? age[0].pct + "%" : "-"}</div><div style={{height: '52px', borderRadius: '6px', background: 'var(--sec)', display: 'flex', alignItems: 'flex-end', overflow: 'hidden'}}><div data-anim="bar" style={{width: '100%', height: `${age[0]?.pct ?? 0}%`, background: 'var(--sec-mid)'}}></div></div><div className="t-meta" style={{color: 'var(--meta)', marginTop: '6px'}}>{age[0]?.label ?? ""}</div></div>
                    <div style={{flex: '1', textAlign: 'center'}}><div style={{fontWeight: '700', fontSize: '13px', color: 'var(--ink)', marginBottom: '6px'}}>{age[1] ? age[1].pct + "%" : "-"}</div><div style={{height: '52px', borderRadius: '6px', background: 'var(--sec)', display: 'flex', alignItems: 'flex-end', overflow: 'hidden'}}><div data-anim="bar" style={{width: '100%', height: `${age[1]?.pct ?? 0}%`, background: '#E8FF66'}}></div></div><div style={{fontSize: '11px', color: 'var(--ink)', fontWeight: '600', marginTop: '6px'}}>{age[1]?.label ?? ""}</div></div>
                    <div style={{flex: '1', textAlign: 'center'}}><div style={{fontWeight: '700', fontSize: '13px', color: 'var(--ink)', marginBottom: '6px'}}>{age[2] ? age[2].pct + "%" : "-"}</div><div style={{height: '52px', borderRadius: '6px', background: 'var(--sec)', display: 'flex', alignItems: 'flex-end', overflow: 'hidden'}}><div data-anim="bar" style={{width: '100%', height: `${age[2]?.pct ?? 0}%`, background: 'var(--sec-mid)'}}></div></div><div className="t-meta" style={{color: 'var(--meta)', marginTop: '6px'}}>{age[2]?.label ?? ""}</div></div>
                    <div style={{flex: '1', textAlign: 'center'}}><div style={{fontWeight: '700', fontSize: '13px', color: 'var(--ink)', marginBottom: '6px'}}>{age[3] ? age[3].pct + "%" : "-"}</div><div style={{height: '52px', borderRadius: '6px', background: 'var(--sec)', display: 'flex', alignItems: 'flex-end', overflow: 'hidden'}}><div data-anim="bar" style={{width: '100%', height: `${age[3]?.pct ?? 0}%`, background: 'var(--sec-mid)'}}></div></div><div className="t-meta" style={{color: 'var(--meta)', marginTop: '6px'}}>{age[3]?.label ?? ""}</div></div>
                  </div>
                </div>
                  ...
                  {contentItems.map((item, itemIdx) => (<React.Fragment key={itemIdx}>
                    <a href={item.url || undefined} target={item.url ? "_blank" : undefined} rel="noopener noreferrer" className="mcard" style={{scrollSnapAlign: 'start', flex: '0 0 62%', overflow: 'hidden'}}>
                      <div style={{position: 'relative', width: '100%', aspectRatio: '9/13'}}>{item.raw.mediaKind === 'video'
                          ...
                          : <Slot url={item.slot} alt={item.name} style={{ width: '100%', height: '100%', display: 'block' }} />}<span style={{position: 'absolute', left: '10px', top: '10px', fontSize: '9px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '.05em', background: 'rgba(255,255,255,.92)', color: 'var(--ink)', borderRadius: '999px', padding: '3px 9px'}}>Reel</span></div>
                ...
                <div className="snap-track" style={{gap: '16px', margin: '22px -20px 0', padding: '2px 20px 6px'}}>
                  {brandItems.map((brand, brandIdx) => (<React.Fragment key={brandIdx}>
                    <a href={brand.url || undefined} target={brand.url ? "_blank" : undefined} rel="noopener noreferrer nofollow" className="mcard" style={{scrollSnapAlign: 'start', flex: '0 0 68%', padding: '18px', display: 'block', textDecoration: 'none', color: 'inherit'}}>
                      {/* objectFit contain, never cover: the auto-fetched marks are favicons,
                          often 32px, and filling this box with one turns a brand's logo
                          into a smudge. Desktop caps it the same way. */}
                      <Slot url={brand.slot} alt={brand.name} initial={brand.name.trim().charAt(0).toUpperCase()} style={{width: '100%', height: '110px', display: 'block', background: '#F1F4FA', borderRadius: '12px', color: '#79809C', objectFit: 'contain'}} />
              ...
              <div className="sr" id="pitch">
                <div style={{borderRadius: '24px', background: 'var(--ink)', color: '#fff', padding: '34px 26px'}}>
                  ...
                  <div style={{display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '28px'}}>
                    ...
                    {data.replyTime && (<div style={{display: 'flex', alignItems: 'center', gap: '10px'}}><span style={{display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '22px', height: '22px', borderRadius: '6px', background: 'var(--neon)', flexShrink: '0'}}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg></span><span style={{fontSize: '13px', color: '#fff'}}>{`Replies in ${data.replyTime}`}</span></div>)}
                  </div>
