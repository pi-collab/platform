...
import './brand-deals-empty.css'

/**
 * Brand deals, empty state, DESKTOP.
 *
 * Replaces a small card that sat UNDER the page's own hero. This export draws
 * the whole screen -- its own "My deals" heading, the three counters, the search
 * and the filter chips -- so rendering it inside the existing hero would show
 * two headings for one page. It stands in for the hero and the card together.
 *
 * The search, the sort and the chips are kept and are inert, as the export ships
 * them: they are the shape of the screen a brand is about to have. The counters
 * read zero because they ARE zero; this renders only when the brand has no deals
 * at all, so nothing here is a placeholder awaiting real data.
 *
 * All CSS is scoped under .bdeals-desk. The export ships `*`, `body`, bare `a`
