...
export default function BriefDetailsToggle({ children, label, subtitle, defaultOpen = false, variant = 'inline' }: BriefDetailsToggleProps) {
    ...
    <div style={isSurface ? undefined : { marginTop: 28, paddingTop: 24, borderTop: '1px solid var(--border-hairline)' }}>
      ...
      >
        <span style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: isSurface ? 20 : 16, fontWeight: 700, letterSpacing: isSurface ? '-0.02em' : '-0.015em', color: 'var(--ink)' }}>{label || 'More details'}</span>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 600, color: 'var(--ink-soft)', flexShrink: 0 }}>
          <span>{open ? (isSurface ? 'Hide' : 'View less') : (isSurface ? 'Show' : 'View more')}</span>
