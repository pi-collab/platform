...
import { submitBrandReview } from './review-actions'
...
export default function BrandReviewCard({ dealId, creatorName, existingRating, existingNote }: Props) {
  ...
  const initials = creatorName.slice(0, 1).toUpperCase()

  const handleSubmit = () => {
    if (rating === 0) return
    startSave(async () => {
      const result = await submitBrandReview(dealId, rating, note || null)
      ...
      <div className="surface" style={{ padding: 'clamp(22px,2.6vw,30px) clamp(24px,3vw,34px)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
        <div style={{ width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <p style={{ fontSize: 12, color: 'var(--ink-faint)', marginTop: 14, fontFamily: 'var(--font-ui)' }}>
            Your rating is private and not shared directly with the creator.
          </p>
        </div>
      </div>
    )
    ...
    <div className="surface" style={{ padding: 'clamp(22px,2.6vw,30px) clamp(24px,3vw,34px)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      <div style={{ width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--ink-faint)', marginTop: 10, fontFamily: 'var(--font-ui)' }}>
          This is not shared directly with the creator.
        </p>
        <button
