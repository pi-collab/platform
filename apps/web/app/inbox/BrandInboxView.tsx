...
export default function BrandInboxView({
  ...
  const [messagesByDeal, setMessagesByDeal] = useState<Record<string, Message[]>>(() => {
    ...
    // Sort ascending within each deal
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    // Mark as known BEFORE adding to state — realtime echo will be skipped
    knownIdsRef.current.add(msg.id)
    setMessagesByDeal((prev) => ({
      ...prev,
    ...
    return (
      <div style={emptyWrap}>
        ...
        <p style={{ fontSize: '0.875rem', color: '#888', margin: 0, lineHeight: 1.6 }}>
          Messages will appear here when you start a deal thread.
        </p>
      </div>
    )
  }
  ...
  return (
    <div style={container}>
      ...
      <div style={{ ...leftPanel, display: mobileChat ? undefined : undefined }} className="inbox-left">
        ...
        <div style={threadList}>
          {threads.map((t) => (
            ...
            >
              {t.creatorPhoto ? (
                <img src={t.creatorPhoto} alt={t.creatorName} style={avatar} />
              ) : (
                <div style={avatarFallback}>
                  {t.creatorName.split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2)}
                </div>
              )}
          ...
          <>
            <div style={chatHeader}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#111', margin: 0 }}>{selectedThread.creatorName}</p>
                <p style={{ fontSize: '0.75rem', color: '#888', margin: '0.1rem 0 0' }}>{selectedThread.dealTitle}</p>
              </div>
              <a href={`/deals/${selectedThread.dealId}`} style={viewDealLink}>View deal</a>
            </div>

            <div style={messagesArea}>
              {selectedMessages.length === 0 ? (
                <p style={emptyChat}>No messages yet — start the conversation.</p>
              ) : (
                <div style={msgList}>
                  {selectedMessages.map((msg) => {
                    ...
                    return (
                      <div key={msg.id} style={{ display: 'flex', justifyContent: isBrand ? 'flex-end' : 'flex-start' }}>
                        <div style={isBrand ? bubbleMine : bubbleTheirs}>
                          <p style={bubbleLabel}>{isBrand ? 'You' : 'Creator'}</p>
