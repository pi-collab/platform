...
**Tier tag on the profile, for BOTH tracks**
- [ ] The creator's own profile identity card carries a tier tag under their handle: **"Guapd Growth"** or **"Guapd Deals"**. Shown to everyone — a tag only Growth creators carry is a mark of not-being-the-other-one; shown to both it is simply which tier you are on
- [ ] The dashboard header slot no longer shows it. That header is for what to do next, and a badge saying which tier you are on is not a thing to do
...
- [ ] The shine is a bright **arc travelling in the ring itself**: a square conic gradient rotated with `transform`, clipped to the 2px border by a masked wrapper. It cannot leave the circumference because the window it shows through IS the circumference
- [ ] **Regression guard — do NOT rewrite this as an animated `@property` angle.** That version is fewer elements and it shipped broken: the CSS minifier strips `@property` from the build (verified: zero occurrences in `.next/static/css`), so `--tier-angle` did not resolve, the conic gradient became invalid, and the shine disappeared entirely rather than degrading. A plain `transform` rotation cannot fail that way — its worst case is a stationary arc
