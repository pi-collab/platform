...
- [ ] **Configure custom SMTP (Resend) for transactional email before launch** — fixes signup confirmation (real, enforced, and rate-limit-blocked today: built-in Supabase email allows only ~4/hr project-wide, shared across confirmations and resets). Does **NOT** fix password reset completion — see the separate item below.
- [x] **~~Build the `/reset-password` page~~ — BUILT.** Was a silent dead-end: `resetPasswordForEmail` redirected to `/auth/callback`, which logged the user in with their **old password unchanged**. Now `/auth/confirm` (`token_hash` + `verifyOtp`) → `/reset-password` → `updateUser({ password })`, guarded by a live session plus an httpOnly one-time recovery-marker cookie, with other sessions revoked on success. Uses `token_hash` rather than the PKCE `code` flow specifically so reset works when the email is opened on a different device than it was requested from. **Needs two dashboard settings to function: `/auth/confirm` on the redirect allowlist, and the Reset Password email template sending `{{ .TokenHash }}`.**
- [ ] **VERIFY: does signup email confirmation have the same cross-device weakness?** Signup confirmation still routes through `/auth/callback`, which uses PKCE `exchangeCodeForSession` and therefore needs the `code_verifier` cookie from the browser that started the flow. If a brand signs up on a laptop and confirms from their phone, it may fail the same way password reset did. **Not yet tested** — flagged during the reset work and deliberately not widened into it. Test: sign up on desktop, open the confirmation email on a phone. If it fails, the fix is the same `/auth/confirm` route with `type=signup` allowed and `emailRedirectTo` pointed at it. Matters because Resend is now wired and the pilot depends on confirmation emails landing.
- [ ] **Confirm `mailer_autoconfirm` stays `false` on production** — brand email confirmation must be enforced.

KNOWN DELIBERATE GAP (not a bug — decide before public launch, fine for a founder-run pilot):
- **Team invites send NO email.** `createInvite` returns an `inviteUrl` which `TeamPage.tsx` copies to the admin's clipboard for manual sending (Slack/WhatsApp/etc.). The token infrastructure itself is solid (64-char hex, expiry, idempotent re-invites) — only delivery is manual. Configuring SMTP will NOT make invites send themselves; that needs wiring. Acceptable while founders run onboarding; revisit when brand admins self-serve and would reasonably expect an invite to send itself.
